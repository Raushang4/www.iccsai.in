...
(function($) {
    ...
    $(document).ready(function() {
		...
		var li=$(".lgx-header .menu-onscroll .lgx-navbar .lgx-nav li a")

        $(window).on('scroll', function () {
            var windowpos = $(window).scrollTop();
            if (windowpos >= pos.top) {
                s.addClass("menu-onscroll");
				dd.css('background','rgba(27,39,61,.95)');
				
            } else {
                s.removeClass("menu-onscroll");
				dd.css('background','white');
            }
        });
        ...
        if($('#lgx-typed-string').length){
            $('#lgx-typed-string').typed({
                strings: ["ICCCA 2018", "IEEE Conference","Galgotias University"],
                // typing speed
                typeSpeed: 10,
